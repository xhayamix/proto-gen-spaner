...
const (
	// パラメータの不正
	ErrorCodeNum_InvalidArgument ErrorCodeNum = 1001
	// サーバー内部エラー
	ErrorCodeNum_Internal ErrorCodeNum = 1002
	// 認証エラー
	ErrorCodeNum_Unauthenticated ErrorCodeNum = 1003
	// アクセス権限なし
	ErrorCodeNum_PermissionDenied ErrorCodeNum = 1004
	// リソースが見つからなかった
	ErrorCodeNum_NotFound ErrorCodeNum = 1005
	// ユーザーが見つからなかった
	ErrorCodeNum_UserNotFound ErrorCodeNum = 2001
	// ユーザーは削除済み
	ErrorCodeNum_UserDeleted ErrorCodeNum = 2002
	// メンテナンス中
	ErrorCodeNum_InMaintenance ErrorCodeNum = 2003
	// アカウント停止中
	ErrorCodeNum_AccountBan ErrorCodeNum = 2004
	// NGワードが含まれている
	ErrorCodeNum_NgWordContains ErrorCodeNum = 2005
	// 無効な日付
	ErrorCodeNum_ShopInvalidDay ErrorCodeNum = 2006
)
...
func (e *ErrorCode) DecodeSpanner(val interface{}) error {
	...
	if !ok {
		return errors.New(fmt.Sprintf("値を文字列変換できずErrorCode.DecodeSpannerに失敗しました。 val = %#v", val))
	}
	i, err := strconv.ParseInt(strVal, 10, 64)
	if err != nil {
		return errors.New(fmt.Sprintf("値を数値変換できずErrorCode.DecodeSpannerに失敗しました。 val = %#v, err = %#v", val, err))
	}
	*e = ErrorCode(i)
	return nil
}
...
func (e ErrorCode) ToNumType() ErrorCodeNum {
	switch e {
	case ErrorCode_InvalidArgument:
		return ErrorCodeNum_InvalidArgument
	case ErrorCode_Internal:
		return ErrorCodeNum_Internal
	case ErrorCode_Unauthenticated:
		return ErrorCodeNum_Unauthenticated
	case ErrorCode_PermissionDenied:
		return ErrorCodeNum_PermissionDenied
	case ErrorCode_NotFound:
		return ErrorCodeNum_NotFound
	case ErrorCode_UserNotFound:
		return ErrorCodeNum_UserNotFound
	case ErrorCode_UserDeleted:
		return ErrorCodeNum_UserDeleted
	case ErrorCode_InMaintenance:
		return ErrorCodeNum_InMaintenance
	case ErrorCode_AccountBan:
		return ErrorCodeNum_AccountBan
	case ErrorCode_NgWordContains:
		return ErrorCodeNum_NgWordContains
	case ErrorCode_ShopInvalidDay:
		return ErrorCodeNum_ShopInvalidDay
	}
	return 0
}
...
func NewErrorCodeSet(es ...ErrorCode) ErrorCodeSet {
	s := make(ErrorCodeSet, len(es))
	for _, e := range es {
		s[e] = struct{}{}
	}
	return s
...
func (e ErrorCodeCommaSeparated) Split() (Enums, []string) {
	...
	for _, str := range list {
		...
		if err != nil {
			errs = append(errs, fmt.Sprintf("enum.ErrorCodeを数値変換できませんでした。 err = %q\n", str))
			continue
		}
...
func (e *ErrorCodeNum) UnmarshalJSON(b []byte) error {
	var s string
	if err := json.Unmarshal(b, &s); err != nil {
		return err
	}
	switch strings.ToLower(s) {
	case "invalidargument":
		*e = ErrorCodeNum_InvalidArgument
	case "internal":
		*e = ErrorCodeNum_Internal
	case "unauthenticated":
		*e = ErrorCodeNum_Unauthenticated
	case "permissiondenied":
		*e = ErrorCodeNum_PermissionDenied
	case "notfound":
		*e = ErrorCodeNum_NotFound
	case "usernotfound":
		*e = ErrorCodeNum_UserNotFound
	case "userdeleted":
		*e = ErrorCodeNum_UserDeleted
	case "inmaintenance":
		*e = ErrorCodeNum_InMaintenance
	case "accountban":
		*e = ErrorCodeNum_AccountBan
	case "ngwordcontains":
		*e = ErrorCodeNum_NgWordContains
	case "shopinvalidday":
		*e = ErrorCodeNum_ShopInvalidDay
	default:
		i, _ := strconv.ParseInt(s, 10, 64)
		*e = ErrorCodeNum(int32(i))
	}
	return nil
}
