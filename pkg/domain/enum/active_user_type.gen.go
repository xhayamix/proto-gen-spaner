...
type ActiveUserTypeNum int32

const (
	// 仮登録
	ActiveUserTypeNum_provisional ActiveUserTypeNum = 1
	// アクティブユーザー
	ActiveUserTypeNum_Active ActiveUserTypeNum = 2
	// アカウント削除済み
	ActiveUserTypeNum_Deleted ActiveUserTypeNum = 99
)
...
func (e *ActiveUserType) DecodeSpanner(val interface{}) error {
	...
	if !ok {
		return errors.New(fmt.Sprintf("値を文字列変換できずActiveUserType.DecodeSpannerに失敗しました。 val = %#v", val))
	}
	i, err := strconv.ParseInt(strVal, 10, 64)
	if err != nil {
		return errors.New(fmt.Sprintf("値を数値変換できずActiveUserType.DecodeSpannerに失敗しました。 val = %#v, err = %#v", val, err))
	}
	*e = ActiveUserType(i)
	return nil
}
...
func (e ActiveUserType) ToNumType() ActiveUserTypeNum {
	switch e {
	case ActiveUserType_provisional:
		return ActiveUserTypeNum_provisional
	case ActiveUserType_Active:
		return ActiveUserTypeNum_Active
	case ActiveUserType_Deleted:
		return ActiveUserTypeNum_Deleted
	}
	return 0
}
...
func NewActiveUserTypeSet(es ...ActiveUserType) ActiveUserTypeSet {
	s := make(ActiveUserTypeSet, len(es))
	for _, e := range es {
		s[e] = struct{}{}
	}
	return s
...
func (e ActiveUserTypeCommaSeparated) Split() (Enums, []string) {
	...
	for _, str := range list {
		...
		if err != nil {
			errs = append(errs, fmt.Sprintf("enum.ActiveUserTypeを数値変換できませんでした。 err = %q\n", str))
			continue
		}
...
func (e *ActiveUserTypeNum) UnmarshalJSON(b []byte) error {
	var s string
	if err := json.Unmarshal(b, &s); err != nil {
		return err
	}
	switch strings.ToLower(s) {
	case "provisional":
		*e = ActiveUserTypeNum_provisional
	case "active":
		*e = ActiveUserTypeNum_Active
	case "deleted":
		*e = ActiveUserTypeNum_Deleted
	default:
		i, _ := strconv.ParseInt(s, 10, 64)
		*e = ActiveUserTypeNum(int32(i))
	}
	return nil
}
