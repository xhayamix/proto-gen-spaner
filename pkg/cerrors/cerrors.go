...
// CustomError サーバ-クライアント間エラーハンドリング用エラー
...
func As(cause error) (*CustomError, bool) {
	var cerr *CustomError
	if errors.As(cause, &cerr) {
		return cerr, true
	}
...
func (e *CustomError) FormatError(p xerrors.Printer) error {
	p.Print(e.Message())
	e.frame.Format(p)
	return e.Unwrap()
}
