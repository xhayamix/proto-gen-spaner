...
import (
	...
	cstrings "github.com/xhayamix/proto-gen-spanner/pkg/util/strings"
)
...
type {{ .PascalName }}Num int32

const (
	{{- range .Elements }}
	{{ if .Comment }} // {{ .Comment }}{{ end }}
	{{ $Name }}Num_{{ .PascalName }} {{ $Name }}Num = {{ .Value }}
	{{- end }}
)
...
func (e *{{ .PascalName }}) DecodeSpanner(val interface{}) error {
	...
	if !ok {
		return errors.New(fmt.Sprintf("値を文字列変換できず{{ .PascalName }}.DecodeSpannerに失敗しました。 val = %#v", val))
	}
	i, err := strconv.ParseInt(strVal, 10, 64)
	if err != nil {
		return errors.New(fmt.Sprintf("値を数値変換できず{{ .PascalName }}.DecodeSpannerに失敗しました。 val = %#v, err = %#v", val, err))
	}
	*e = {{ .PascalName }}(i)
	return nil
}
...
func (e {{ .PascalName }}) ToNumType() {{ .PascalName }}Num {
	switch e {
	{{- range .Elements }}
	case {{ $Name }}_{{ .PascalName }}:
		return {{ $Name }}Num_{{ .PascalName }}
	{{- end }}
	}
	return 0
}
...
func New{{ .PascalName }}Set(es ...{{ .PascalName }}) {{ .PascalName }}Set {
	s := make({{ .PascalName }}Set, len(es))
	for _, e := range es {
		s[e] = struct{}{}
	}
	return s
...
func (e {{ .PascalName }}CommaSeparated) Split() (Enums, []string) {
	...
	for _, str := range list {
		...
		if err != nil {
			errs = append(errs, fmt.Sprintf("enum.{{ .PascalName }}を数値変換できませんでした。 err = %q\n", str))
			continue
		}
...
func (e *{{ .PascalName }}Num) UnmarshalJSON(b []byte) error {
	var s string
	if err := json.Unmarshal(b, &s); err != nil {
		return err
	}
	...
	{{- range .Elements }}
	case "{{ .LowerName }}":
		*e = {{ $Name }}Num_{{ .PascalName }}
	{{- end }}
	default:
		i, _ := strconv.ParseInt(s, 10, 64)
		*e = {{ .PascalName }}Num(int32(i))
	}
	return nil
}
