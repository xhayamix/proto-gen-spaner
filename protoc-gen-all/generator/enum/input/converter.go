...
import (
	...
	options "github.com/xhayamix/proto-gen-spanner/pkg/domain/proto/definition/options/enums"
...
func ConvertMessageFromProto(file *protogen.File) (*Enum, error) {
	...
	isPreference := strings.Contains(ret.SnakeName, "preference_type")
	for _, element := range define.GetElements() {
		...
		ret.Elements = append(ret.Elements, &Element{
			...
			SnakeName:           core.ToSnakeCase(element.GetName()),
