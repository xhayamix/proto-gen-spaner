...
func createMessage(
	...
	for _, field := range message.Fields {
		...
		case input.TypeKind_Enum:
			importPathSet.Add("server/enums/enums_gen.proto")
		...
		case field.PkgType == input.PkgType_APICommon:
			if field.ImportFileName != "" {
				importPathSet.Add("server/api/common/" + strings.ReplaceAll(field.ImportFileName, ".proto", "_gen.proto"))
			}
			typeName = "api.common." + typeName
		case field.PkgType == input.PkgType_ServerCommon:
			if field.ImportFileName != "" {
				importPathSet.Add("server/common/" + strings.ReplaceAll(field.ImportFileName, ".proto", "_gen.proto"))
			}
			typeName = "server.common." + typeName
		case field.ImportFileName != "":
			importPathSet.Add("server/api/" + strings.ReplaceAll(field.ImportFileName, ".proto", "_gen.proto"))
		}
		if field.IsList {
		...
		if field.FieldOption.MasterRef != nil {
			importPathSet.Add("server/master/common/options.proto")
		...
		if field.HiddenOption {
			importPathSet.Add("server/options/zap.proto")
...
func (c *Creator) Create(file *input.File) (*output.TemplateInfo, error) {
	...
	/*
		if len(emptyResponses) > 0 || responseMessageNameSetNeedCommonResponse.Size() > 0 {
			importPathSet.Add("server/api/common/response_gen.proto")
		}
	*/
